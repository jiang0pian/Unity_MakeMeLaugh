...
{
    private float distance = 3.0f;              // Nail放置距离玩家的距离
    private float NailDamage = 1.0f;            // Nail的伤害
    private Rigidbody2D jellyRb;                // Nail的刚体组件
    ...
    void Start()
    {
        // 获取实例化Nail的Rigidbody2D组件
        jellyRb = this.GetComponent<Rigidbody2D>();
        maxHealth = 1;
        currentHealth = 1;

        // 确保Nail对象拥有Rigidbody组件以使其受到重力影响并能够坠落至地面。
        if (jellyRb != null)
        {
            jellyRb.mass = 1000000; // 设置一个很大的质量值
            jellyRb.gravityScale = 1; // 确保Nail受到重力影响，可以根据需要调整
        }
    }
