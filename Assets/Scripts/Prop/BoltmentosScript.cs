...
public class BoltmentosScript : Prop
{


    private float distance = 0.0f;              // Boltmentos放置距离玩家的距离
    public Rigidbody2D anchor;                  // 锚点的刚体
    private Rigidbody2D rb;                     // Boltmentos的刚体
    private Collider2D coll;                    // Boltmentos的碰撞器
    private bool isPressed;                     // 鼠标左键是否按下
    private bool isCatapult;                    // Boltmentos是否被发射
    ...
    private void Start()
    {
        ...
        rb.isKinematic = true;                          // 使Boltmentos运动状态变为不受外力
        coll.isTrigger = true;                          // 使Boltmentos碰撞不可用，仅作为触发器
