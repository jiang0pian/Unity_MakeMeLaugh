...
{
    private float distance = 3.0f;              // Rigidcolajelly放置距离玩家的距离
    private float jellyForce = 150.0f;          // Rigidcolajelly的弹力
    private Rigidbody2D jellyRb;                // Rigidcolajelly的刚体组件
    ...
    void Start()
    {
        // 获取实例化Rigidcolajelly的Rigidbody2D组件
        jellyRb = this.GetComponent<Rigidbody2D>();
        maxHealth = 1;
        currentHealth = 1;

        // 确保Rigidcolajelly对象拥有Rigidbody组件以使其受到重力影响并能够坠落至地面。
        if (jellyRb != null)
        {
